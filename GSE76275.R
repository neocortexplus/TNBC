...
gset <- getGEO("GSE76275", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
# Create the plot
ggplot(data_to_plot, aes(x = group, y = AGR3, color = group)) +
  geom_boxplot() +
  labs(title = "Expression of AGR3 in TNBC and Normal Samples",
       x = "Sample Group",
       y = "Expression Level") +
