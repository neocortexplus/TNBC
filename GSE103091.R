...
data <- read_excel("/home/aiusrdata/RCode/TNBC/GSE103091.xls",col_names = TRUE)

headers <- names(data)

if (!"Characteristics_os" %in% headers) {
  data$Characteristics_os <- NA
}

for (i in 1:(nrow(data) - 1)) {
  if (!is.na(data[i, "Group"]) && data[i, "Group"] == "-") {
    if (is.na(data[i + 1, "Group"]) && grepl("os \\(days\\):", data[i + 1, "Characteristics"])) {
      data[i, "Characteristics_os"] <- data[i + 1, "Characteristics"]
    }
  }
}
...
data <- data %>%
  mutate(
    mfs_days = as.numeric(str_extract(Characteristics, "\\d+")),  # Extract numeric part and convert to numeric
    label = if_else(mfs_days < 1095, 0, 1)  # Categorize based on the condition
  )

# data <- data %>%
#   mutate(
#     os_days = as.numeric(str_extract(Characteristics_os, "\\d+")),  # Extract numeric part and convert to numeric
#     label = if_else(os_days < 1095, 0, 1)  # Categorize based on the condition
#   )
...
gset <- getGEO("GSE103091", GSEMatrix =TRUE, getGPL=TRUE)
...
gsms <- group_string <- paste0(data$label, collapse = "")
...
df <- merge(t_ex, data, by.x = "row.names", by.y = "Accession")
...
surv_data <- pData(phenoData(gset))

data <- read_excel("/home/aiusrdata/RCode/TNBC/GSE58812.xls",col_names = TRUE)
...
data <- data %>%
  mutate(
    mfs_days = as.numeric(str_extract(Characteristics, "\\d+")),  # Extract numeric part and convert to numeric
    group = if_else(mfs_days < 1095, 0, 1)  # Categorize based on the condition
  )

data <- data %>%
  rename(
    time_to_event = mfs_days,
    event = Death,
    Age = `Age at diag`
  )

data$time_to_event <- as.numeric(gsub(".*?(\\d+)", "\\1", data$time_to_event))
data$event <- as.numeric(data$event)
data$Age <- as.numeric(data$Age)


# Prepare the survival data
surv_data  <- data[, c("time_to_event", "event", "Age")]


# Create the survival object
surv_obj <- with(surv_data, Surv(time_to_event, event == 1))

# Plot Kaplan-Meier survival curve
km_plot <- ggsurvplot(
  survfit(surv_obj ~ 1, data = surv_data),
  data = surv_data,
  xlab = "Time (days)",
...
cox_model <- coxph(Surv(time_to_event, event == 1) ~ Age , data = surv_data)

# Summary of the Cox model
summary(cox_model)

# Visualize the impact of covariates
cox_plot <- ggforest(cox_model, data = surv_data, fontsize = 1.5)

print(cox_plot)

# Stratified Kaplan-Meier curves for different groups, e.g., by age group or tumor size
strat_km_plot <- ggsurvplot(
  survfit(surv_obj ~ cut(Age, breaks = quantile(Age, probs = c(0, 0.5, 1), na.rm = TRUE)), data = surv_data),
  data = surv_data,
...
complete <- merge(gene_data, data, by.x = "row.names", by.y = "Accession")
