...
gset <- getGEO("GSE86945", GSEMatrix =TRUE, getGPL=TRUE)
if (length(gset) > 1) idx <- grep("GPL17586", attr(gset, "names")) else idx <- 1
gset <- gset[[idx]]
...
temp_pd <- pData(phenoData(temp))


common_genes <- Reduce(intersect, list(rownames(GSE38959_ex), rownames(GSE76250_ex), rownames(temp_ex)))



GSE38959_ex_common <- GSE38959_ex[common_genes, ]
GSE76250_ex_common <- GSE76250_ex[common_genes, ]
temp_ex_common <- temp_ex[common_genes, ]



all <- cbind(GSE38959_ex_common, GSE76250_ex_common , temp_ex_common)

all_fs <- GSE38959_fs[common_genes,]

dim(all_fs);dim(GSE38959_ex_common) ; dim(GSE76250_ex_common);dim(temp_ex_common);dim(all )
...
all_columns <- Reduce(union, list(names(GSE38959_pd), names(GSE76250_pd), names(temp_pd)))
...
# Repeat for temp
missing_columns_temp <- setdiff(all_columns, names(temp_pd))
temp_full <- temp_pd
for (col in missing_columns_temp) {
  temp_full[[col]] <- NA
}

# Ensure the columns are in the same order
temp_full <- temp_full %>% select(all_of(all_columns))



# Now rbind them together
all_pd <- rbind(GSE38959_full, GSE76250_full,temp_full)
...
batch <- c(rep(0, ncol(GSE38959_ex_common)), rep(1, ncol(GSE76250_ex_common)),rep(2,ncol(temp_ex_common)))
batch <-   factor(batch)
...
# Add legend
legend("topright", inset=c(-0.15,0), legend=c("GSE38959", "GSE76250", "temp"), pch=20,
       col=1:3, title="Group", pt.cex=1.5)  # Updated 'col=1:2' to 'col=1:3' to include three colors
...
temp_gsms <- paste0(rep("1", ncol(temp_ex_common)), collapse = "")

gsms <- paste0(GSE38959_gsms, GSE76250_gsms,temp_gsms)
