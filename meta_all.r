...
temp1_ex <- exprs(temp1)
temp2_ex <- exprs(temp2)
# temp3_ex <- exprs(temp3)
# temp4_ex <- exprs(temp4)


temp1_fs <- fData(temp1)
temp2_fs <- fData(temp2)
# temp3_fs <- fData(temp3)
# temp4_fs <- fData(temp4)


temp1_pd <- pData(phenoData(temp1))
temp2_pd <- pData(phenoData(temp2))
# temp3_pd <- pData(phenoData(temp3))
# temp4_pd <- pData(phenoData(temp4))

expression_matrices <- list(temp1_ex, temp2_ex)

common_genes <- rownames(expression_matrices[[1]])

for (i in 2:length(expression_matrices)) {
  common_genes <- intersect(common_genes, rownames(expression_matrices[[i]]))
}

for (i in 1:length(expression_matrices)) {
  assign(paste0("temp", i, "_ex_common"), expression_matrices[[i]][common_genes, ])
}


all <- get("temp1_ex_common")
for (i in 2:length(expression_matrices)) {
  all <- cbind(all, get(paste0("temp", i, "_ex_common")))
}


all_fs <- temp1_fs[common_genes,]

dim(all_fs);dim(temp1_ex_common) ; dim(temp2_ex_common);dim(all )
...
for (i in 2:length(phenotype_data)) {
  all_columns <- union(all_columns, names(phenotype_data[[i]]))
}


# temp 1
missing_columns_temp1 <- setdiff(all_columns, names(temp1_pd))
temp1_full <- temp1_pd
for (col in missing_columns_temp1) {
  temp1_full[[col]] <- NA
}

# Ensure the columns are in the same order
temp1_full <- temp1_full %>% select(all_of(all_columns))

# Repeat for temp2
missing_columns_temp2 <- setdiff(all_columns, names(temp2_pd))
temp2_full <- temp2_pd
for (col in missing_columns_temp2) {
  temp2_full[[col]] <- NA
}
...
all_pd <- rbind(temp1_full, temp2_full)
...
common_expression_matrices <- list(temp1_ex_common, temp2_ex_common)
batch <- c()

for (i in 1:length(common_expression_matrices)) {
  num_columns <- ncol(common_expression_matrices[[i]])
  batch <- c(batch, rep(i - 1, num_columns))
}
